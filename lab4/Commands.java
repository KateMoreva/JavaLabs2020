...
class Commands {
    ...
    private static void putCangePrice(String[] args) throws Exception{
        if (args.length == 3) {
            ...
            if (args[1].equals("--help") || (args[1].equals("-h"))) {
                System.out.println("Команда 'change_price' позволяет изменить цену товара."
                        + "\n" + "Для изменения стоимости введите команду, название продукта, новую стоимость и нажмите 'Enter'."
                        + "\n" + "Пример: change_price товар10 10000");
            } else
                throw new Exception("Неправильное число аргументов.");
        }
    }
    ...
    private static void putFilterPrice(String[] args) throws Exception{
        if (args.length == 3 ) {
            ...
            if (args[1].equals("--help") || (args[1].equals("-h"))) {
                System.out.println("Команда 'filter_by_price' позволяет просмотреть товары в заданном ценовом диапазоне цен."
                        + "\n" + "Для просмотра товаров введите команду, минимальную и максимальную цену и нажмите 'Enter'."
                        + "\n" + "Пример: filter_by_price 1000 10000");
            } else
                throw new Exception("Неправильное число аргументов.");
        }
    }
